...
void draw(){
  ...
  for(int y = 1; y < gray.height-1; y++){ 
    for(int x = 1; x < gray.width-1; x++){ 
      sum = red(im.get(x-1,y-1)) + (red(im.get(x  ,y-1))*2) + red(im.get(x+1,y-1)) //平均化フィルタのコードに重み付けの計算を追加
          + (red(im.get(x-1,y  ))*2) + (red(im.get(x  ,y  ))*4) + (red(im.get(x+1,y  ))*2) 
          + red(im.get(x-1,y+1)) + (red(im.get(x  ,y+1))*2) + red(im.get(x+1,y+1)); 
      gray.set(x, y, color(sum/16));                // 16個の画素値の平均値を設定 
    } 
  }
